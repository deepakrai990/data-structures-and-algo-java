...
/**
 * @created 16/09/22 12:33 pm
 * @project Scaler_Beginners_Module
 * @author Deepak Kumar Rai
 */
public class Q6_Sum_of_All_Subarrays {
    /**
     ...
     * So, for each element, let X be the number of elements to their left, and Y be the number of elements
     * to their right.
     ...
     * **/
    private static long subarraySum1(int[] A) {
        long ans = 0;
        int n = A.length;
        for (int i = 0; i < n; i++)
    ...
    private static long subarraySum2(int[] A) {
        long total_sum = 0;
        int N = A.length;
        for(int i = 0; i < N; i++){
            int sum = 0;
    ...
    public static void main(String[] args) {
        int[] A = {1, 2, 3};
        long res =  subarraySum(A);
        System.out.println("My Approach subarraySum1() Result : " + res);
        res = subarraySum1(A);
        System.out.println("Scalar Approach subarraySum() Result : " + res);
        res = subarraySum2(A);
        System.out.println("My Approach 2 subarraySum1() Result : " + res);
