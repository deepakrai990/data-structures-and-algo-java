...
#### Problem Description
```text
Find the contiguous non-empty subarray within 
an array, A of length N, with the largest sum.
```
#### Problem Constraints
```text
1 <= N <= 1e6
...
possible sum of the contiguous subarray.
```
#### Example Input
```text
Input 1:
...
public class Solution {
    ...
    public int maxSubArray(final List<Integer> A) {
        ...
        int maxSum = Integer.MIN_VALUE; // maximum subarray sum for all sub arrays till now
	    for (int num : A) {
	        curSum += num;
