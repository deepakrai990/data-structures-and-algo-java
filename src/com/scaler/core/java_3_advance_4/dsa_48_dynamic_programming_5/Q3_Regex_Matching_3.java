...
public class Q3_Regex_Matching_3 {
    ...
    // Function to preprocess the pattern and replace consecutive '*' with a single '*'
    private static String preprocessPattern(String pattern) {
        StringBuilder sb = new StringBuilder();
        sb.append(pattern.charAt(0));
        for (int i = 1; i < pattern.length(); i++) {
            if (pattern.charAt(i) == '*' && pattern.charAt(i - 1) == '*') {
                continue; // Skip consecutive '*' characters
            }
